...
def convolution(X, K, stride):
    rs = cs = X.itemsize
    X_slices = np.lib.stride_tricks.as_strided(X, shape = (K.shape[0],K.shape[1], X.shape[0] - K.shape[0] + 1, X.shape[1]-K.shape[1]+1), strides = (X.shape[0]*rs,cs,rs,X.shape[1]*cs))
...
#######################################################################
def NN(EPOCH, ALPHA, filter_dim, num_filters):
    ...
    W = (2*np.random.random_sample((num_outputs, input_dim - filter_dim + 1, input_dim - filter_dim + 1, num_filters)) - 1) / 1000
